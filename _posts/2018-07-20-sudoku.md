...
I have used prime numbers as colors in my code. The reason being that, when I visit a node, I have to make sure the adjacent nodes are not of the same color. In order to ensure that, I have maintained a variable "color" for each node. It is initialized to NO_COLOR which is again a prime number. While visiting a node(A), if the adjacent node(B) is NO_COLOR then, it is changed to the color of the node(A) and if the node(B) is already colored, then we multiply the color of node(A) to the color of node(B). The color of the node has another property(variable) True or False. The variable being True if the color of the node is already set. 
